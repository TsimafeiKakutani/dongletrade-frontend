...
function Statistic() {
  ...
  useEffect(() => {
    ...
    const fetchData = async (coin2: any, searchString: any) => {
      ...
      for (let i = 0; i < result?.data?.Data?.length; i++) {
        ...
        allCoinPair['dollarprice'] = result?.data?.Data[i]?.DollarPrice
  ...
  const viewType = [
    ...
    {
      title: '24h High',
      value: numberWithCommas(headerInfos?.High),
    },
    { title: '24h Low', value: numberWithCommas(headerInfos?.Low) },
    {
    ...
    <>
      <Box display="flex" className={classes.coinPairForm}>
        ...
        <Box alignSelf="center">
          ...
          >
            {numberWithCommas(headerInfos?.Price)}
          </Typography>
          <Typography variant="body1" className={clsx(classes.fontColor1, classes.text4)}>
            ${numberWithCommas(headerInfos?.DollarPrice)}
          </Typography>
        </Box>
      </Box>
