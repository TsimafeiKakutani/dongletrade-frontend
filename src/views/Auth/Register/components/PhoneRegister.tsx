...
function PhoneForm() {
  ...
  const handleRegisterSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    if (isValid && serviceCheck !== false && password.length > 7 && upperCnt !== 0 && numberCnt !== 0) {
    ...
    <>
      <form onSubmit={handleRegisterSubmit}>
        <FormControl className={classes.formControlSide} variant="outlined">
          <FormHelperText className={clsx(classes.fontColor1, classes.helperText)}>Full Name</FormHelperText>
          <ClickAwayListener onClickAway={() => setClickName(false)}>
            ...
            >
              ...
              {name.length > 0 && clickName === true ? (
                <CancelIcon className={classes.icon} onClick={() => setName('')} />
              ) : (
                ''
              )}
            </div>
          </ClickAwayListener>
          <FormHelperText className={classes.error}>{nameValid === '' ? '' : nameValid}</FormHelperText>
        </FormControl>
