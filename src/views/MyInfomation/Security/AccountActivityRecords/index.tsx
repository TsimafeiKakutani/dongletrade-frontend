...
import {
  ...
  securityActivityCnt,
} from 'store/accountrecord/selectors'
...
function DeviceManagement() {
  ...
  const [date, setDate] = useState('1 DAYS')
  ...
  const loginTotalCnt = useAppSelector(loginActivityCnt)
  const securityRecords = useAppSelector(securityActivityRecords)
  const securityTotalCnt = useAppSelector(securityActivityCnt)

  useEffect(() => {
    if (tabKey === 'Login Activity') {
      setRowsCnt(loginTotalCnt)
      setRows(loginRecords)
    } else {
      setRowsCnt(securityTotalCnt)
      setRows(securityRecords)
    }
  }, [loginRecords, loginTotalCnt, securityRecords, securityTotalCnt, tabKey])
